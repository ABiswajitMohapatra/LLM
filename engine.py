...
# Detects an explicit "what's on page N" style question so it can bypass
# semantic/keyword search entirely and go straight to a direct metadata
# lookup - literal page numbers rarely appear meaningfully inside chunk
# text, so embedding/BM25 search alone is unreliable for this intent.
PAGE_REQUEST_PATTERN = re.compile(
    r"\b(?:page|slide)s?\s*(?:no\.?|number|num)?\s*#?\s*(\d+)\b|\b(\d+)\s*(?:st|nd|rd|th)?\s*(?:page|slide)\b",
    re.IGNORECASE
)
...
- You may be given "Document Context" (from the user's own uploaded/indexed files) and/or
  ...
  - STRICT GROUNDING RULE: when answering from Document Context, use ONLY facts that actually
    appear in it. Do not invent, assume, infer beyond what's stated, or fill gaps with outside/
    general knowledge for that claim. If the Document Context doesn't contain the answer (or only
    partially covers it), say so plainly (e.g. "The provided document doesn't mention that") rather
    than guessing - this applies even if you happen to know the answer from training data.
...
def extract_pages_from_pdf(file):
    """
    Page-aware PDF extraction for RAG indexing (citations need real page
    numbers). Kept separate from extract_text_from_pdf() so that function's
    existing callers/behavior are completely untouched.
    Returns a list of (page_number, text) tuples, 1-indexed. Falls back to
    per-page OCR for scanned pages with little/no embedded text.
    """
    try:
        file.seek(0)
    except Exception:
    ...
    if total_chars < 30 * max(len(pages), 1):
        # Likely a scanned document - OCR each page as an image.
        try:
            import pypdfium2 as pdfium
            try:
                file.seek(0)
            ...
            for i in range(len(pdf_doc)):
                if pages[i][1]:
                    continue  # this page already had real embedded text
                bitmap = pdf_doc[i].render(scale=3)
                ocr_text = pytesseract.image_to_string(bitmap.to_pil())
                pages[i][1] = ocr_text.strip()
        except Exception:
            pass
...
def extract_pages_from_pptx(file):
    """Page-aware PPTX extraction, one "page" per slide, for RAG indexing."""
    if Presentation is None:
        raise UnsupportedFileError(
            "PPTX support isn't available on the server (python-pptx not installed)."
        )
    try:
        prs = Presentation(file)
    except Exception as e:
        raise UnsupportedFileError(f"Couldn't read this .pptx file: {e}")
...
def load_file_pages(path_or_buffer, filename: str):
    """
    Page-aware counterpart to load_file(), used only by the RAG store for
    indexing uploaded documents (citations need page numbers). File types
    without a natural page concept (docx, txt/code, images) are returned
    as a single page (page number None). Accepts raw bytes or a
    file-like object, same as load_file().
    """
    buf = io.BytesIO(path_or_buffer) if isinstance(path_or_buffer, (bytes, bytearray)) else path_or_buffer
    ext = filename.lower().rsplit(".", 1)[-1] if "." in filename else ""
    if ext == "pdf":
        return extract_pages_from_pdf(buf)
    if ext == "pptx":
        return extract_pages_from_pptx(buf)
    # Everything else: reuse load_file() and treat as one unpaginated page.
    text = load_file(buf, filename)
...
def extract_text_from_docx(file) -> str:
    if docx is None:
        raise UnsupportedFileError(
            "DOCX support isn't available on the server (python-docx not installed)."
        )
    try:
        document = docx.Document(file)
    except Exception as e:
        raise UnsupportedFileError(f"Couldn't read this .docx file: {e}")

    parts = [p.text for p in document.paragraphs]

    # Tables carry a lot of real content in most .docx documents (specs,
    # comparison sheets, invoices) - without this, that text is silently
    # dropped even though the file "extracted successfully".
...
def extract_text_from_doc(file) -> str:
    """
    Legacy binary .doc (pre-2007 Word format) - python-docx can't read
    this (it only handles the .docx zip/XML format). There's no pure-Python
    parser available here, so we shell out to `antiword` if it happens to
    ...
    raise UnsupportedFileError(
        ...
        "Please re-save/export the file as .docx and upload again."
    )
...
def load_file(path_or_buffer, filename: str) -> str:
    """
    Detects file type from its extension and routes to the right extractor.
    Raises UnsupportedFileError with a clear, user-facing message when a
    file is a recognized-but-unreadable format (corrupt, legacy .doc, etc)
    or a genuinely unsupported extension - callers (the /upload route)
    should catch this and show it to the user instead of a silent failure.
    """
    ext = filename.lower().rsplit(".", 1)[-1] if "." in filename else ""
    if ext == "pdf":
    ...
    if ext == "doc":
        return extract_text_from_doc(path_or_buffer)
    if ext == "pptx":
        return extract_text_from_pptx(path_or_buffer)
    if ext == "ppt":
        raise UnsupportedFileError(
            "Legacy .ppt files aren't supported for text extraction. "
            "Please re-save/export the file as .pptx and upload again."
        )
    ...
    if ext in ("png", "jpg", "jpeg", "webp"):
        ...
        return extract_text_from_image(path_or_buffer, mime_type=mime_map[ext])
    raise UnsupportedFileError(
        f"'.{ext or '?'}' files aren't supported. Supported formats: "
        "PDF, DOC, DOCX, PPT (.pptx only), PPTX, PNG, JPG, JPEG, WEBP, "
...
# Ordered from coarsest to finest boundary - recursive splitter tries each
# in turn, only falling through to a finer (more aggressive) separator when
# the current one isn't found in a piece that's still too big.
_SPLIT_SEPARATORS = ["\n\n", "\n", ". ", "! ", "? ", "; ", ", ", " "]


def _recursive_split(s: str, chunk_size: int, seps: list):
    if len(s) <= chunk_size:
        return [s] if s.strip() else []
    if not seps:
        # No natural boundary left to split on - hard character cut is the
        # only remaining option (same as the old behavior, but now only
        # used as a last resort instead of always).
        return [s[i:i + chunk_size] for i in range(0, len(s), chunk_size) if s[i:i + chunk_size].strip()]

    sep, rest = seps[0], seps[1:]
    parts = s.split(sep)
    if len(parts) == 1:
        return _recursive_split(s, chunk_size, rest)
    ...
    for i, part in enumerate(parts):
        ...
        if len(candidate) <= chunk_size:
            ...
            if len(piece) > chunk_size:
                groups.extend(_recursive_split(piece, chunk_size, rest))
                current = ""
            else:
                current = piece
    if current.strip():
...
def chunk_text(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP):
    """
    Recursive character-based splitter (same strategy as LangChain's
    RecursiveCharacterTextSplitter): tries progressively finer separators -
    paragraph break, line break, sentence end, clause, word - so chunks end
    on natural boundaries instead of mid-sentence/mid-word. Falls back to a
    hard character cut only if no separator fits within chunk_size.
    A character-based overlap is then stitched onto the start of each
    chunk (after the first) so context isn't lost at chunk boundaries -
    same overlap semantics as before, just applied post-split.
    """
    text = re.sub(r"[ \t]+", " ", text).strip()
    if not text:
        return []

    raw_chunks = [c.strip() for c in _recursive_split(text, chunk_size, _SPLIT_SEPARATORS) if c.strip()]
    if not raw_chunks:
        return []
    ...
    for c in raw_chunks[1:]:
        ...
        chunks.append((prev_tail + " " + c).strip())
    return chunks
...
def start_background_task(query: str, model: str | None = None, persona_prompt: str | None = None):
    ...
    def _run():
        try:
            ...
            generator, doc_sources, web_used, _citations = chat_with_agent_stream(
...
def build_prompt(query, index, chat_history, memory_limit=14, extra_file_content="",
...
Reply briefly and naturally (1-2 sentences)."""
    ...
    if uploaded_docs:
        ...
        # is actually about, then either hand over sampled full-document
        # coverage (for summary/overview requests, which need the whole
        # document rather than a handful of semantically-matched fragments)
        # or the most relevant chunks (for narrow factual questions), tagged
        # per-source + per-page so the model can cite exactly where each
        # fact came from.
        ...
        # rag_store is imported lazily here (not at module top) to avoid a
        # hard circular-ish dependency at import time - it in turn imports
        # this module for the embedder/chunker/file loaders.
        import rag_store

        if SUMMARY_REQUEST_PATTERN.search(query) and not PAGE_REQUEST_PATTERN.search(query):
            per_doc_cap = 6000 if len(resolved_ordered) == 1 else 3000
            for fname in resolved_ordered:
                text = uploaded_docs.get(fname, "")
                if fname.lower().endswith(".pptx"):
                    # PPT decks must never lose slides to the char cap or to
                    # chunk sampling - always hand over every extracted slide
                    # (extract_text_from_pptx already includes all of them)
                    # instead of the PDF-oriented cap/sample path below.
                    file_context_parts.append(f"[Source: {fname}]\n{text}")
                elif len(text) <= per_doc_cap or fname not in rag_store.list_indexed():
                    # Small doc, or not yet indexed (e.g. indexing still
                    # running in the background) - use raw text as before.
                    file_context_parts.append(f"[Source: {fname}]\n{text[:per_doc_cap]}")
                else:
                    # Large doc: sample chunks evenly across its full length
                    # instead of only ever seeing the first per_doc_cap
                    # characters, so summaries stay accurate on 300-500 page
                    # documents rather than describing only the intro.
                    sampled = rag_store.sample_document_chunks(fname, max_chunks=14)
                    for s in sampled:
                        citations.append({"filename": fname, "page": s["page"], "snippet": s["text"][:200]})
                    block = "\n\n".join(
                        f"(Page {s['page']}) {s['text']}" if s["page"] else s["text"]
                        for s in sampled
                    )
                    file_context_parts.append(f"[Source: {fname}]\n{block[:per_doc_cap]}")
        else:
            # Targeted retrieval mode: hybrid (embedding + BM25) search
            # against the persisted RAG store - no re-embedding of the
            # whole document on every single message.
            #
            # BUG FIX: this used to call rag_store.hybrid_search()
            # unconditionally, even for files whose background indexing
            # (kicked off on /upload) hadn't finished yet - for a large
            # PDF that can take a while the first time (embedding model
            # download + embedding hundreds of chunks). During that
            # window the file simply isn't in the FAISS index yet, so
            # hybrid_search correctly found nothing and the user got a
            # false "I don't have that page" even though the document was
            # uploaded fine. Now, per file, we check whether indexing has
            # actually completed (same check the summary branch already
            # used) and fall back to a direct on-the-fly search of the
            # raw extracted text for any file that isn't indexed yet, so
            # answers are correct immediately, not just once indexing
            # eventually catches up.
            by_file = {}

            # Page-specific queries: try direct metadata lookup FIRST, before
            # any FAISS/vector-store readiness check. get_chunks_by_page()
            # reads chunk-level page metadata directly and does not depend
            # on embeddings/FAISS at all, so this must not be gated behind
            # "is this file in the vector store yet".
            page_match = PAGE_REQUEST_PATTERN.search(query)
            if page_match:
                requested_page = int(page_match.group(1) or page_match.group(2))
                for fname in resolved_ordered:
                    page_chunks = rag_store.get_chunks_by_page(fname, requested_page)
                    if page_chunks:
                        by_file[fname] = page_chunks
                        for c in page_chunks:
                            citations.append({"filename": fname, "page": requested_page, "snippet": c["text"][:200]})
                    else:
                        progress = rag_store.get_progress(fname)
                        if progress.get("status") == "done":
                            lo, hi = rag_store.get_page_range(fname)
                            print(f"[RAG] Page {requested_page} not found in '{fname}' "
                                  f"(indexed page range: {lo}-{hi}).")
                            if lo is not None:
                                page_range_note += (
                                    f"\n(Note: '{fname}' is indexed with pages {lo}-{hi}. "
                                    f"Page {requested_page} does not exist in this document - "
                                    f"tell the user this directly instead of guessing.)"
                                )
                        else:
                            # Metadata for this page genuinely isn't
                            # available yet (extraction/embedding for this
                            # file hasn't reached completion) - be honest
                            # instead of guessing.
                            page_range_note += (
                                f"\n(Note: '{fname}' is still being indexed in the background "
                                f"({progress.get('status')}, {progress.get('percent')}%) - "
                                f"page-specific lookups aren't available yet for this file. "
                                f"Tell the user indexing is still in progress and to try again "
                                f"in a moment, rather than guessing at page content.)"
                            )

            indexed_files = rag_store.list_indexed()
            ready = [f for f in resolved_ordered if f in indexed_files]
            not_ready = [f for f in resolved_ordered if f not in indexed_files]
            if not_ready:
                print(f"[RAG] build_prompt: {not_ready} not yet in vector store "
                      f"(background indexing still running or failed) - "
                      f"falling back to direct search of raw text for these.")

            if ready and not (page_match and by_file):
                results = rag_store.hybrid_search(ready, query, top_k=max(TOP_K, 5))
                for r in results:
                    by_file.setdefault(r["filename"], []).append(r)
                    citations.append({"filename": r["filename"], "page": r["page"], "snippet": r["text"][:200]})

            for fname in not_ready:
                if page_match:
                    # Already handled above via direct metadata lookup +
                    # an informative note - a plain cosine-similarity
                    # search over raw text (no page metadata) would just
                    # return whatever chunks are textually closest to the
                    # words "page 45", which has nothing to do with actual
                    # page 45.
                    continue
                text = uploaded_docs.get(fname, "")
                ...
                by_file[fname] = [{"text": file_chunks[i], "page": None} for i in top_idx]
                for i in top_idx:
                    citations.append({"filename": fname, "page": None, "snippet": file_chunks[i][:200]})
            ...
            for fname in resolved_ordered:
                ...
                if not chunks:
                    continue
                chunk_block = "\n\n".join(
                    f"(Page {c['page']}) {c['text']}" if c["page"] else c["text"]
                    for c in chunks
                )
                file_context_parts.append(f"[Source: {fname}]\n{chunk_block[:3000]}")
...
{"Chunks above are tagged '(Page N)' where available - cite the filename and page number when you use them, e.g. 'network_security.pdf, Page 183'." if citations else ""}
{"Answer strictly using the Document Context above. Do not add facts, numbers, or details that are not present in it. If it doesn't contain the answer, say so directly instead of guessing." if doc_context else ""}
...
def chat_with_agent_stream(query, index, chat_history, memory_limit=14, extra_file_content="",
                            uploaded_docs=None, last_uploaded=None, last_uploaded_batch=None, model=None,
                            persona_prompt=None, memory_context=None):
    prompt, doc_sources, web_used, _, citations = build_prompt(
        query, index, chat_history, memory_limit, extra_file_content,
        uploaded_docs, last_uploaded, last_uploaded_batch
    )
    return (stream_groq(prompt, model=model, persona_prompt=persona_prompt, memory_context=memory_context),
            doc_sources, web_used, citations)
